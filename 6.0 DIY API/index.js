...
app.get("/random", (req,res)=>{
  var random = jokes[Math.floor(Math.random()*100)].jokeText
  res.json(random)
})
...
app.get("/jokes/:id", (req,res)=>{
  ...
  var jokeF = jokes.find((joke)=>joke.id === id)
  res.json(jokeF)
})
...
app.get("/filter", (req,res)=>{
  ...
  var jokeF = jokes.filter((joke)=>joke.jokeType === query)
  res.json(jokeF)
})
...
app.patch("/jokes/:id", (req,res)=>{
  ...
  var correctedJoke = {
    id: id,
    jokeText: req.body.text || jokeF.jokeText,
    jokeType: req.body.type || jokeF.jokeType
  }
...
app.delete("/all", (req,res)=>{
  ...
  if (userKey == masterKey) {
    ...
    res.status(404).json({error: `You are not authorized to do this task`})
  }
